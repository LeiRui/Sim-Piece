...
public class MySample2 {
  ...
  // java -jar MySample2-jar-with-dependencies.jar 0.005 HouseTwenty_TEST false -1 0 "D:\\kdd\\lts-exp\\datasets\\%s.csv" "D:\\kdd\\lts-exp\\notebook\\SimPieceResults\\%s-%s-%s.csv"
  public static void main(String[] args) {
    double epsilonFactor = Double.parseDouble(args[0]);
...
//    for (int k = 0; k < datasetNames.length; k++) {
    ...
    try (FileInputStream inputStream = new FileInputStream(
      ...
      TimeSeries ts = TimeSeriesReader.getMyTimeSeries(inputStream, delimiter, false, N,
          start, hasHeader);
      double epsilon = ts.range * epsilonFactor;
...
//        System.out.println(epsilon);
      ...
      System.out.println(
          datasetName + ": n=" + N + ",epsilon=" + epsilon + ",m=" + segments.size() * 2);
      segments.sort(Comparator.comparingLong(SimPieceSegment::getInitTimestamp));
      try (PrintWriter writer = new PrintWriter(
          new FileWriter(String.format(csvFile, datasetName, N, epsilon)))) {
